...
class MasahubCrawler(Crawler):
    ...
    async def video(self, scrape_item: ScrapeItem) -> None:
        ...
        download_url = self.parse_url(Selector.VIDEO_SRC(soup))
        title = css.select_one_get_text(soup, Selector.TITLE)
        _, ext = self.get_filename_and_ext(download_url.name)
        custom_filename = self.create_custom_filename(title, ext)
        return await self.handle_file(
            download_url, scrape_item, download_url.name, ext, custom_filename=custom_filename
        )
